/**
 * NON-OPERATIONAL RESEARCH ONLY.
 * Luma monetary / issuance simulation model for economic research.
 * This module is NOT part of the Civizen production application and must NOT be
 * imported from `src/`. Production UI uses `src/lib/prototype-credits.ts`
 * (format/parse helpers + prototype notice only).
 */

/** Controlling interim notice for Luma prototype status (institutional correction v0.2). */
export const LUMA_PROTOTYPE_NOTICE =
  'Luma is a prototype feature used for demonstration and internal product testing. It is not money, a security, a deposit, a stablecoin, a claim on assets, or a promise of financial value. It cannot be purchased, withdrawn, redeemed, or transferred outside Civizen. Peer and marketplace transfers are disabled.' as const;
...
export function executeApprovedIssuance(input: {
  ...
  // Production path retired: Luma is a non-transferable prototype demonstration unit only.
  void input;
  throw new Error(
    'Superseded model: operational Luma issuance is disabled. Prototype credits are demonstration-only.',
  );
}
