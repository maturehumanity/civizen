...
} from './governance-activation-demographic-worker';
import {
...
export async function runActivationDemographicFeedWorkerSweep(args: {
  ...
  for (const adapter of args.adapters) {
    ...
    try {
      ...
      const isPayloadError = lowerMessage.includes('payload') || lowerMessage.includes('target_population') || lowerMessage.includes('target population');
