...
export interface KeyStorageProvider {
   ...
   */
  storeKey(keyId: string, key: Uint8Array, metadata?: Record<string, unknown>): Promise<void>;
...
export class DevKeyStorageProvider implements KeyStorageProvider {
  ...
  private storagePrefix = 'civizen_key_';
...
export class iOSSecureEnclaveProvider implements KeyStorageProvider {
  ...
  async retrieveKey(keyId: string): Promise<Uint8Array | null> {
    // Implementation would use:
    // import * as Keychain from 'react-native-keychain';
    // const credentials = await Keychain.getGenericPassword({ service: 'com.civizen.identity' });
  ...
  async deleteKey(keyId: string): Promise<void> {
    // Implementation would use:
    // import * as Keychain from 'react-native-keychain';
    // await Keychain.resetGenericPassword({ service: 'com.civizen.identity' });
    throw new Error('iOS implementation requires native module');
  }
...
export function getKeyStorageProvider(): KeyStorageProvider {
  ...
  type ReactNativeWebViewBridge = { platform?: string };
  const rnWebView = typeof window !== 'undefined'
    ? (window as Window & { ReactNativeWebView?: ReactNativeWebViewBridge }).ReactNativeWebView
    : undefined;
  if (rnWebView) {
    // Running in React Native WebView
    const platform = rnWebView.platform;
    if (platform === 'ios') {
