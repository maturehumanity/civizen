...
export const APP_UPDATE_CHANNEL_KEY = 'civizen-app-update-channel';
export const APP_UPDATE_CHANNEL_EVENT = 'civizen-app-update-channel-changed';
...
function persistChannel(next: StoredAppUpdateChannel) {
  if (!isBrowser()) return;
  try {
...
export function setAppUpdateChannel(channel: AppUpdateChannel) {
  persistChannel(buildStoredChannel(channel));
}

/** Force Live when the user is not allowed to use the Test channel. */
export function ensureAuthorizedAppUpdateChannel(allowTesting: boolean): AppUpdateChannel {
  const current = getAppUpdateChannel();
  if (current === 'testing' && !allowTesting) {
    setAppUpdateChannel('release');
    return 'release';
  }
  return current;
}
