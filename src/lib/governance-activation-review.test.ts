...
describe('governance activation review helpers', () => {
  ...
  it('maps readiness labels for legacy activation enums', () => {
    expect(getActivationStatusLabel('pre_activation')).toBe('Exploratory');
    expect(getActivationStatusLabel('pending_review')).toBe('Readiness review');
    expect(getActivationStatusLabel('approved_for_activation')).toBe('Approved for program availability');
    expect(getActivationStatusLabel('activated')).toBe('Program available');
    expect(getActivationStatusLabel('rejected')).toBe('Not approved');
    expect(getActivationStatusLabel('revoked')).toBe('Availability withdrawn');
    expect(getActivationStatusLabel('future_status' as Parameters<typeof getActivationStatusLabel>[0])).toBe(
      'future_status',
    );

    expect(getActivationDecisionLabel('approve')).toBe('Approve readiness');
    expect(getActivationDecisionLabel('reject')).toBe('Do not approve');
    expect(getActivationDecisionLabel('request_changes')).toBe('Request readiness changes');
    expect(getActivationDecisionLabel('declare_activation')).toBe('Approve program availability');
    expect(getActivationDecisionLabel('revoke_activation')).toBe('Withdraw program availability');
    expect(
      getActivationDecisionLabel('future_decision' as Parameters<typeof getActivationDecisionLabel>[0]),
    ).toBe('future_decision');

    for (const decision of ['approve', 'reject', 'request_changes', 'declare_activation', 'revoke_activation'] as const) {
      expect(getActivationDecisionLabel(decision).toLowerCase()).not.toMatch(/activation/);
    }
  });
