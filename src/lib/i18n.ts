...
function readPackedLanguage(language: LanguageCode): TranslationTree | null {
  ...
  try {
    ...
    if (!raw) return null;
    return scrubLegacyBrandTree(JSON.parse(raw)) as TranslationTree;
  } catch {
    return null;
  }
}
...
function persistPackedLanguage(language: LanguageCode, messages: TranslationTree) {
  ...
  try {
    window.localStorage.setItem(
      getLanguageStorageKey(language),
      JSON.stringify(scrubLegacyBrandTree(messages)),
    );
  } catch {
...
export async function loadLanguagePack(language: LanguageCode): Promise<TranslationTree> {
  ...
  if (cached) {
    const scrubbed = scrubLegacyBrandTree(cached) as TranslationTree;
    cachedLanguagePacks.set(language, scrubbed);
    return scrubbed;
  }
