...
/**
 * Compatibility note: table/enum names retain `activation_*` for schema stability.
 * User-facing copy maps these to readiness / program-availability wording (P1-03).
 * Do not create new schema using activation terminology.
 */
...
/** Internal enum values retain activation_* names; present with readiness labels. */
...
/** Presentation-only labels for legacy activation_* status values. */
export function getActivationStatusLabel(status: ActivationReviewStatus) {
  switch (status) {
    ...
    case 'approved_for_activation':
      return 'Approved for program availability';
    case 'activated':
...
export function getActivationDecisionLabel(decision: ActivationReviewDecision) {
  ...
  switch (decision) {
    case 'approve':
      return 'Approve readiness';
    case 'reject':
      return 'Do not approve';
    case 'request_changes':
      return 'Request readiness changes';
    case 'declare_activation':
      return 'Approve program availability';
    case 'revoke_activation':
      return 'Withdraw program availability';
    default:
      return decision;
  }
}
...
/**
 * Maps legacy activation_* status enums to i18n keys under governanceHub.activationReview.
 * Keys keep activation_* suffixes for compatibility; English values use readiness wording.
 */
