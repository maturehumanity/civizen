...
export function getCountryName(code: string, locale: string): string {
  ...
  if (!upper) return '';
  // Intl.DisplayNames only accepts ISO 3166-1 alpha-2 regions; codes like GLOBAL throw.
...
export function getCountryCodeFromName(name: string): string | null {
  ...
  for (const code of COUNTRY_CODES) {
    let label = code.toLowerCase();
    try {
      label = (englishDisplayNames.of(code) || code).trim().toLowerCase();
    } catch {
      // Skip codes the runtime cannot resolve.
    }
    if (label === normalized) {
      return code;
    }
