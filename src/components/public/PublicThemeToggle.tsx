...
export function PublicThemeToggle({ className }: PublicThemeToggleProps) {
  ...
  const toggleOpposite = () => {
    ...
    // Keep options visible while the pointer remains over the control.
    ...
    >
      <PopoverTrigger asChild>
        <Button
          ...
          aria-expanded={open}
          className={cn(
            'group relative h-9 w-9 shrink-0 border-border/60 bg-card/80',
            className,
          )}
          onMouseEnter={() => {
          ...
          onPointerDown={(event) => {
            // Desktop: hover opens the menu; click should only flip the theme.
