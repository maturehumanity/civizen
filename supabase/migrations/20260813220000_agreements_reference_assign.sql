-- Peek next Agreement number without consuming it; allow a requested reference on create.

CREATE OR REPLACE FUNCTION public.peek_agreement_next_reference()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  END IF;
  SELECT last_value INTO v_n
  FROM public.agreement_reference_counters
  WHERE year = v_year;
  v_n := coalesce(v_n, 0) + 1;
  RETURN jsonb_build_object(
    'year', v_year,
    'sequence', v_n,
    'referenceCode', 'AGR-' || v_year::text || '-' || lpad(v_n::text, 4, '0')
  );
END;
$$;

CREATE OR REPLACE FUNCTION public.agreement_assign_reference(p_requested text)
RETURNS text
LANGUAGE plpgsql
AS $$
DECLARE
  v_requested text := nullif(trim(p_requested), '');
  v_year integer;
  v_n integer;
BEGIN
  IF v_requested IS NULL THEN
    RETURN public.agreement_next_reference();
  END IF;
  IF EXISTS (SELECT 1 FROM public.agreements WHERE reference_code = v_requested) THEN
    RETURN public.agreement_next_reference();
  END IF;
  IF v_requested ~ '^AGR-[0-9]{4}-[0-9]+$' THEN
    v_year := substring(v_requested from 5 for 4)::integer;
    v_n := nullif(ltrim(substring(v_requested from 10), '0'), '')::integer;
    IF v_n IS NULL THEN
      v_n := 0;
    END IF;
    ...
    ON CONFLICT (year) DO UPDATE
      SET last_value = GREATEST(public.agreement_reference_counters.last_value, EXCLUDED.last_value);
  END IF;
  RETURN v_requested;
END;
$$;
...
BEGIN
  ...
  INSERT INTO public.agreements (
    ...
    'draft',
    public.agreement_assign_reference(p_payload->>'reference_code'),
    v_title,
    v_type,
...
GRANT EXECUTE ON FUNCTION public.peek_agreement_next_reference() TO authenticated;
GRANT EXECUTE ON FUNCTION public.agreement_assign_reference(text) TO authenticated;
