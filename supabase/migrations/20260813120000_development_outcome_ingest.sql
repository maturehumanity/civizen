...
BEGIN
  ...
  ON CONFLICT (source_story_key) DO UPDATE
    SET
      ...
      area = EXCLUDED.area,
      created_features = CASE
        WHEN EXCLUDED.created_features IS NOT NULL AND cardinality(EXCLUDED.created_features) > 0
          THEN EXCLUDED.created_features
        ELSE public.development_stories.created_features
      END,
      ...
      source_id = COALESCE(EXCLUDED.source_id, public.development_stories.source_id),
      source_url = COALESCE(EXCLUDED.source_url, public.development_stories.source_url),
      chat_id = COALESCE(EXCLUDED.chat_id, public.development_stories.chat_id),
      commit_sha = COALESCE(EXCLUDED.commit_sha, public.development_stories.commit_sha),
      pr_number = COALESCE(EXCLUDED.pr_number, public.development_stories.pr_number),
      confidence_score = COALESCE(EXCLUDED.confidence_score, public.development_stories.confidence_score),
      metadata = COALESCE(public.development_stories.metadata, '{}'::jsonb) || COALESCE(EXCLUDED.metadata, '{}'::jsonb),
...
COMMENT ON FUNCTION public.ingest_development_story(text, text, text, text, text, text, text[], text, text, timestamptz, text, text, text, text, text, text, text, text, integer, numeric, jsonb) IS
  'Authenticated development-story ingest. Resolves author to profiles.id. Merges metadata on conflict so outcome evidence can accumulate.';
