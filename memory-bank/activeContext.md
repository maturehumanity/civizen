...
**Profile Earnings page (2026-08-03 / 2026-08-04):** `/earnings` in the Home avatar menu shows seller product/service activity from agreements (signed = sold; pending listed separately). Illustrative Lumen totals only — settlement not active. Notice describes earnings from work, services, and products (no registration/banking roadmap language). Filters use a single-line scrollable underline tab strip (not pills). Home removed from that menu (bottom nav covers it); `pageRegistry.home` icon aligned to Lucide `Home`. LANGUAGE_PACK_VERSION 115. **Testing v0.1.139** (build 141).
