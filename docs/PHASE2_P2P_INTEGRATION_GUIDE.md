...
peerDiscovery.addPeer({
  id: 'peer-1',
  addresses: ['http://peer1.civizen.local:8765'],
  lastSeen: Date.now(),
  reputation: 50,
  type: 'gun',
});
...
High-level API for syncing all Civizen data types.
...
```typescript
// .env.staging
REACT_APP_GUN_PEERS=http://staging-gun.civizen.local:8765/gun
REACT_APP_IPFS_API=http://staging-ipfs.civizen.local:5001
REACT_APP_IPFS_GATEWAY=http://staging-ipfs.civizen.local:8080
