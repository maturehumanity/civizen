...
| Progressive disclosure | Show decision-relevant fields first; advanced/metadata on demand. Do **not** re-display identity the signed-in member already provided (name, phone, age, email, country) — use those values silently and show the field only when it is missing. |
| Outlined form fields | Text inputs and textareas use a rounded outline with the **label on the upper-left border** (`OutlinedField`). Do not stack a label above a dashed underline unless the user gives explicit layout instructions for that surface. |
