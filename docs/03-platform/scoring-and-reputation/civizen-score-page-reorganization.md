...
Contribution-level evaluation (`contribution-evaluation-v3`) is distinct from Contributions category reputation. A contribution observation answers how significant a specific verified outcome was, including bounded human contribution substance; `reputationFromObservations` still produces the category score (prior 50 / strength 6). Individual observations must not be shown as “+N Civizen Score.” Realized impact, structural significance, and human substance stay separate; missing impact stays unknown. Canonical roots are inspectable on `/profile/contributions`. Do not retune Score V2 formulas to hit a target number.
