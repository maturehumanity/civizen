...
- The app links that key to a decentralized identifier and a Civizen citizen profile.
...
- governance legitimacy survives if the original Civizen-hosted infrastructure disappears
...
That path gives Civizen a realistic route from startup-style administration to citizen-verifiable governance without pretending that absolute invulnerability is achievable.
