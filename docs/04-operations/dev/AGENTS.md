...
- **2026-08 correction — Home post composer dead click:** Failure pattern: wraparound `contentEditable` + floated avatar left the empty field with a short hit target (`min-h-[1.5rem]` vs 40–48px avatar) and no chrome focus path, so taps on placeholder/padding/avatar did nothing. Mandatory: empty composer editor min-height must cover the avatar row; field chrome (`data-home-post-composer`) must focus the editor on mouse/pointer down via `focusHomePostComposerFromChrome`; avatar in the composer is `pointer-events-none`. Stop condition: do not ship Home composer layout changes without `src/lib/home-post-composer-focus.test.ts` + `src/pages/Home.composer.test.tsx` passing.
