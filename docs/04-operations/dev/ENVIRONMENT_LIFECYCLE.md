...
Civizen is open source, but open contribution is not the same as open deployment. Code, schema, and release artifacts move through explicit gates.
...
For now, Civizen uses one public web origin (`https://civizen.world`) and separate Android update manifests for Test and Live.

- **Default:** every app checks the **Live** (`android-release`) manifest.
- **Test channel** is permission-gated (`updates.test`) for founders, admins, system, and explicitly granted testers. The Settings Live/Test switcher is hidden for everyone else.
- Unauthorized devices that somehow have Test selected are forced back to Live.
- Test selection still auto-returns to Live after a short window.
- Sideload updates download the APK in-app and open the system installer (with browser fallback).
- Production remains the stable fallback if a Testing build fails to boot.
...
Supabase remains the operational source of truth while Civizen is still early-stage. It provides authentication, row-level security, transactional writes, realtime subscriptions, and the migration history the app already depends on.
