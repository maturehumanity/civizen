...
const allowProductionChange = process.env.CIVIZEN_ALLOW_PRODUCTION_RELEASE_CHANGE === 'true';
...
if (productionReleaseFiles.length > 0 && !allowProductionChange) {
  ...
  console.error('Set CIVIZEN_ALLOW_PRODUCTION_RELEASE_CHANGE=true only in an approved production release job.');
  process.exit(1);
}
