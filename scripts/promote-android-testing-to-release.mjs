...
const origin = 'https://civizen.world';
...
function writeUpdateJs(filePath, manifest) {
  fs.writeFileSync(
    filePath,
    `window.__CIVIZEN_ANDROID_UPDATE__ = ${JSON.stringify(manifest, null, 2)};\n`,
    'utf8',
  );
}
...
function main() {
  ...
  const testingApkName = `civizen-debug-testing-${releaseId}.apk`;
  const releaseApkName = `civizen-debug-release-${releaseId}.apk`;
